...
# ────────────────────────── Binance REST API (replaces blocked WebSocket) ────
# Binance WebSocket returns HTTP 451 (geo-block) on Railway/cloud IPs.
# We use the public REST klines endpoint instead — no IP restrictions.
...
BINANCE_POLL_INTERVAL: int = 10   # fetch klines every N seconds
...
# ══════════════════════════════════════════════════════════════════════════════
#  BINANCE REST POLLER  (replaces WebSocket — HTTP 451 geo-block on Railway)
# ══════════════════════════════════════════════════════════════════════════════

async def binance_ws_listener() -> None:
    """
    Poll Binance REST /api/v3/klines every BINANCE_POLL_INTERVAL seconds for
    BTC, ETH, SOL 5-minute candle data.

    Why REST instead of WebSocket?
      Binance WebSocket returns HTTP 451 (geo/legal block) when connecting
      from cloud datacenter IPs (Railway, Heroku, AWS, etc.).
      The public REST endpoint does not have this restriction.

    Updates STATE.klines[asset] identically to the old WebSocket path so
    the rest of the bot is unaffected.
    """
    log.info("Binance REST poller started (interval: %ds).", BINANCE_POLL_INTERVAL)
                ...
                for asset, symbol in BINANCE_SYMBOLS.items():
                    try:
                        params = {
                            "symbol":   symbol,
                            "interval": "5m",
                            "limit":    2,      # current + last closed candle
                        }
                        async with session.get(
                            BINANCE_REST_URL,
                            params=params,
                            ...
                            if resp.status != 200:
                                log.warning(
                                    "Binance REST [%s] HTTP %s", asset, resp.status
                                )
                                continue
                            data = await resp.json()
                        ...
                        # Use the most recent (in-progress) candle — index -1
                        k = data[-1]
                        ...
                        STATE.klines[asset] = {
                            ...
                            "low":          float(k[3]),
                            "close":        float(k[4]),
                            "volume_quote": float(k[7]),   # quote asset volume (USD)
                            "is_closed":    False,          # in-progress candle
                            "open_time":    int(k[0]),
                            "close_time":   int(k[6]),
                            "ts":           time.time(),
                        }
                        log.debug(
                            "Binance REST [%s] close=%.4f vol=$%.0f",
                            asset, float(k[4]), float(k[7]),
                        )

                    except Exception as asset_exc:
                        log.warning("Binance REST error [%s]: %s", asset, asset_exc)

                await asyncio.sleep(BINANCE_POLL_INTERVAL)

            except Exception as exc:
                log.warning("Binance REST poller error: %s — retrying in 15 s …", exc)
...
async def main() -> None:
    ...
    Task map:
      • binance_ws_listener   — REST kline polling from Binance (no geo-block)
