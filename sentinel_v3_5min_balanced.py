...
async def evaluate_market(
    ...
    mispricing = true_prob - poly_prob

    # Determine side and EV
    if mispricing > 0 and abs(mispricing) >= 0.10 and true_prob > 0.35:
        side = "YES"
        ev = compute_ev(poly_prob, true_prob)
    elif mispricing < 0 and abs(mispricing) >= 0.10 and true_prob < 0.65:
        side = "NO"
        poly_no = max(1.0 - poly_prob, 0.001)
        true_no = max(1.0 - true_prob, 0.001)
        ev = compute_ev(poly_no, true_no)
    else:
        log.debug("[%s] No edge: mispricing=%.2f true=%.2f", asset, mispricing, true_prob)
        return None
    ...
    # Full diagnostic log — one line per market showing all key metrics
    log.info(
        "[%s] %s | poly=%.2f true=%.2f misprice=%.2f EV=%.0f%% bayes=%.2f | %s $%.0f",
        asset, side,
        poly_prob, true_prob, mispricing,
        ev * 100, posterior,
        market.get("_direction","?"), market.get("_threshold", 0),
    )

    if ev < MIN_EV:
        log.info("[%s] ❌ EV=%.0f%% < %.0f%%", asset, ev*100, MIN_EV*100)
        return None

    bayes_threshold = 0.65 if PAPER_MODE else MIN_BAYESIAN
    if posterior < bayes_threshold:
        log.info("[%s] ❌ Bayes=%.2f < %.2f", asset, posterior, bayes_threshold)
        return None
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        ...
        while True:
            try:
                ...
                if not signals:
                    log.debug("Cycle: %d markets, 0 signals", len(all_markets))
                    continue
