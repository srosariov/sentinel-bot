...
async def evaluate_market(
    ...
    if vol_1h_approx < MIN_1H_VOLUME_USD:
        log.debug("[%s] Volume filter failed: $%.0f < $%.0f", asset, vol_1h_approx, MIN_1H_VOLUME_USD)
        return None
    ...
    if ev < MIN_EV:
        log.debug("[%s] EV filter failed: %.3f (poly=%.3f true=%.3f)", asset, ev, poly_prob, true_prob)
        return None
    ...
    log.info(
        "✅ SIGNAL PASSED ALL FILTERS: [%s] %s | price=$%.2f %s $%.2f "
        "| EV=%.1f%% | Bayes=%.1f%% | OB=%.3f | poly_prob=%.3f | true_prob=%.3f",
        asset, question[:50],
        market.get("_current_price", 0), market.get("_direction", "?"),
        market.get("_threshold", 0),
        ev * 100, posterior * 100, ob_imb, poly_prob, true_prob,
    )
    return {
        "asset":         asset,
        "market_id":     market_id,
