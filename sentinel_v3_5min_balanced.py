...
async def evaluate_market(
    ...
    # In paper mode use relaxed Bayesian threshold to generate test signals
    bayes_threshold = 0.65 if PAPER_MODE else MIN_BAYESIAN
    if posterior < bayes_threshold:
        log.info("[%s] ❌ Bayesian: %.3f < %.3f", asset, posterior, bayes_threshold)
        return None
