...
PRICE_POLL_INTERVAL: int = 15   # fetch prices every N seconds
...
# ══════════════════════════════════════════════════════════════════════════════
#  PRICE POLLER — CoinGecko (works from Railway/cloud IPs, no API key needed)
# ══════════════════════════════════════════════════════════════════════════════

async def binance_ws_listener() -> None:
    """
    Poll CoinGecko public API every PRICE_POLL_INTERVAL seconds for
    BTC, ETH, SOL price + 24h volume data.

    Why CoinGecko instead of Binance?
      Binance blocks ALL connections (WebSocket AND REST) with HTTP 451
      from cloud datacenter IPs (Railway us-west-2, AWS, Heroku, etc.)
      due to US legal restrictions. CoinGecko has no such block.

    Data mapped into STATE.klines[asset] in the same format the signal
    engine expects — no other code changes needed.

    Volume approximation:
      CoinGecko returns 24h total volume. We divide by 288 (number of
      5-minute periods in 24h) to approximate 5m volume, then multiply
      by 12 to get the 1h estimate used by the volatility filter.
    """
    log.info("CoinGecko price poller started (interval: %ds).", PRICE_POLL_INTERVAL)

    # CoinGecko simple/price endpoint — single call for all 3 assets
    url = f"{COINGECKO_BASE}/simple/price"
    coin_ids = ",".join(COINGECKO_IDS.values())

    # For OHLC we use the /coins/{id}/ohlc endpoint (1-day, gives recent candles)
    ohlc_url_tmpl = COINGECKO_BASE + "/coins/{coin_id}/ohlc"

    connector = aiohttp.TCPConnector(limit=5)
    async with aiohttp.ClientSession(connector=connector) as session:
        # Track previous price per asset to build open/close
        prev_price: dict = {}

        while True:
            try:
                # ── Step 1: Fetch current prices + 24h volume ─────────────────
                params = {
                    "ids":            coin_ids,
                    "vs_currencies":  "usd",
                ...
                async with session.get(
                    ...
                    if resp.status == 429:
                        log.warning("CoinGecko rate-limited — waiting 30 s …")
                        await asyncio.sleep(30)
                        continue
                    if resp.status != 200:
                        log.warning("CoinGecko price API HTTP %s", resp.status)
                        await asyncio.sleep(PRICE_POLL_INTERVAL)
                        continue
                    price_data = await resp.json()

                # ── Step 2: Build kline dict for each asset ───────────────────
                for asset, coin_id in COINGECKO_IDS.items():
                    coin = price_data.get(coin_id, {})
                    if not coin:
                        continue

                    current_price = float(coin.get("usd", 0))
                    vol_24h       = float(coin.get("usd_24h_vol", 0))
                    change_24h    = float(coin.get("usd_24h_change", 0))

                    if current_price == 0:
                        continue

                    # Approximate OHLC from current + previous price
                    prev = prev_price.get(asset, current_price)
                    open_p  = prev
                    close_p = current_price
                    high_p  = max(open_p, close_p) * (1 + abs(change_24h) / 100 / 48)
                    low_p   = min(open_p, close_p) * (1 - abs(change_24h) / 100 / 48)

                    # Approximate 5m volume from 24h, then 1h = 12 × 5m
                    vol_5m  = vol_24h / 288
                    vol_1h  = vol_5m * 12   # this is what the volatility filter uses
                    ...
                    STATE.klines[asset] = {
                        ...
                        "low":          low_p,
                        "close":        close_p,
                        "volume_quote": vol_5m,    # 5m USD volume approximation
                        "vol_1h":       vol_1h,    # explicit 1h volume for filter
                        "is_closed":    False,
                        "open_time":    int(time.time() - PRICE_POLL_INTERVAL),
                        "close_time":   int(time.time()),
                        "ts":           time.time(),
                    }
                    prev_price[asset] = current_price

                    log.debug(
                        "CoinGecko [%s] price=$%.2f vol_1h=$%.0f",
                        asset, current_price, vol_1h,
                    )

                log.info(
                    "Prices updated — BTC=$%.0f ETH=$%.0f SOL=$%.2f",
                    STATE.klines.get("BTC", {}).get("close", 0),
                    STATE.klines.get("ETH", {}).get("close", 0),
                    STATE.klines.get("SOL", {}).get("close", 0),
                )
...
async def evaluate_market(
    ...
    # Use explicit 1h vol if provided by CoinGecko poller, else estimate from 5m
    vol_1h_approx = kline.get("vol_1h", vol_5m_quote * 12)
