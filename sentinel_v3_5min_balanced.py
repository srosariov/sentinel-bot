...
async def evaluate_market(
    ...
    # We now scan markets up to 24h out (not just last 180s).
    # The "lag" is detected via price-vs-threshold, not by time window alone.
    # We still require the market to have some time left (> 60s).
    if seconds_left <= 60:
        return None
    ...
    if vol_1h_approx < MIN_1H_VOLUME_USD:
        log.info("[%s] ❌ Volume filter: $%.0f < $%.0f",
    ...
    if ob_imb < MIN_OB_IMBALANCE:
        log.info("[%s] ❌ OB imbalance: %.3f < %.3f", asset, ob_imb, MIN_OB_IMBALANCE)
        return None

    # ── 4. Polymarket implied probability from best ask ───────────────────────
    # Best ask price on YES token ≈ market's implied probability of YES
    if asks:
        try:
    ...
    # If true_prob=0.85 and poly_prob=0.40 → EV = (0.85-0.40)/0.40 = +112.5%
    # That's the market lagging to update after price moved through threshold
    ev = compute_ev(poly_prob, true_prob)
    if ev < MIN_EV:
        log.info("[%s] ❌ EV filter: %.3f (poly=%.3f true=%.3f)",
                  asset, ev, poly_prob, true_prob)
        return None
    ...
    if posterior < MIN_BAYESIAN:
        log.info("[%s] ❌ Bayesian: %.3f < %.3f", asset, posterior, MIN_BAYESIAN)
        return None

    # ── 8. Kelly sizing ───────────────────────────────────────────────────────
    size_usd = kelly_size(ev, posterior, STATE.bankroll)
    if size_usd < 1.0:
        log.info("[%s] ❌ Size too small: $%.2f", asset, size_usd)
        return None
