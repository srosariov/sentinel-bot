#!/usr/bin/env python3
# ═══════════════════════════════════════════════════════════════════════════════
#  bot.py — Sentinel Polymarket v8.3  "Live-Ready: balanced activity + safety"
# ───────────────────────────────────────────────────────────────────────────────
#  Assets:     7 (BTC, ETH, SOL, XRP, DOGE, HYPE, BNB)
#  Timeframes: 1h y 4h ÚNICAMENTE (sin 5m/15m)
#  Búsqueda:   SLUG DETERMINÍSTICO (primario) + BROADCAST (siempre activo).
#              Ambos métodos corren cada ciclo. Slug se ejecuta primero y
#              popula seen_ids; broadcast luego añade mercados que slug-lookup
#              no encontró (slugs no-estándar). Deduplicación por market_id
#              previene duplicados.
#  Filtro:     Duración real estricta — 1h: 45–80min | 4h: 180–300min
...
#    1. Rangos de duración relajados (más mercados reales aceptados):
#         1h:  50–75   →  45–80   min
#         4h:  200–280 →  180–300 min
#    2. Broadcast fallback ACTIVO siempre (antes solo si slug vacío).
#       Slug sigue siendo prioritario; broadcast captura los mercados con
#       slugs no-estándar que slug-lookup no encuentra.
#    3. EV_NORMAL: 4.0% → 3.5%  (más señales sin abandonar filtro de calidad)
# ───────────────────────────────────────────────────────────────────────────────
#  CAMBIOS v8.1+ preservados:
#    - resolve_by_price() honesta — sin random.random()
#    - EXPIRED como categoría separada en stats
#    - MIN_LIQUIDITY $50, EV_RELAXED 2.0%, BAYES 55%/50%
# ═══════════════════════════════════════════════════════════════════════════════
...
EV_NORMAL     = float(os.getenv("EV_NORMAL",     "0.035"))   # 3.5%
...
TIMEFRAMES: Dict[str, Dict] = {
    "1h": {
        ...
        "duration_min_strict": (45,  80),    # 45–80 min para 1h
        "label":              "1 hora",
    },
    "4h": {
        "minutes":            240,
        ...
        "duration_min_strict": (180, 300),   # 180–300 min para 4h
        "label":              "4 horas",
    },
}
...
def main():
    ...
    while True:
        try:
            ...
            tg.stop()
            tg.send("🛑 Sentinel v8.3 detenido manualmente.")
            discord("🛑 Sentinel v8.3 detenido manualmente.")
            break
