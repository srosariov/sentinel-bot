#!/usr/bin/env python3
# ═══════════════════════════════════════════════════════════════════════════════
#  bot.py — Sentinel Polymarket v8.1  "Lag Arbitrage 1h / 4h — Honest Paper"
# ───────────────────────────────────────────────────────────────────────────────
#  Assets:     7 (BTC, ETH, SOL, XRP, DOGE, HYPE, BNB)
#  Timeframes: 1h y 4h  →  más volumen, más edge real, menos ruido
#  Búsqueda:   SLUG DETERMINÍSTICO → /events?slug=  (fix definitivo)
#              Broadcast fallback  → /markets con filtro de fechas
#  Análisis:   Post-trade automático (por qué ganó/perdió + sugerencia)
#  Telegram:   Pure HTTP con auto-reconnect y backoff exponencial
#  Deploy:     Railway — archivo principal bot.py
# ───────────────────────────────────────────────────────────────────────────────
#  CAMBIOS v8.1 vs v8.0:
#    1. MIN_LIQUIDITY default: $100 → $50  (capturar mercados pequeños buenos)
#    2. EV_NORMAL default:    4.5% → 4.0%  (más señales sin ser agresivo)
#    3. resolve_by_price():   ya NO usa random.random() como fallback.
#                             Sin precio real → el trade queda OPEN para
#                             reintento en próximos ciclos. Solo si pasa
#                             el grace period sin precio se cierra como
#                             EXPIRED (no LOSS) usando bet_price como proxy.
#                             Esto preserva la integridad del paper trading.
# ───────────────────────────────────────────────────────────────────────────────
#  POR QUÉ 1h/4h SUPERA A 5m/15m:
#    - 5x–10x más liquidez por mercado
#    - Movimientos de precio significativos (no solo ruido)
#    - El modelo de fade tiene más señal estadística
#    - Menos slippage, mejor pricing
#    - Las apuestas pueden mantenerse abiertas con tiempo de reacción
# ═══════════════════════════════════════════════════════════════════════════════
...
MIN_LIQUIDITY      = float(os.getenv("MIN_LIQUIDITY",  "50.0"))    # USD min

# Filtros de señal (menos conservadores que v7.x para 1h/4h)
EV_NORMAL     = float(os.getenv("EV_NORMAL",     "0.040"))   # 4.0%
EV_RELAXED    = float(os.getenv("EV_RELAXED",    "0.020"))   # 2.0%
BAYES_NORMAL  = float(os.getenv("BAYES_NORMAL",  "0.550"))   # 55%
...
TIMEFRAMES: Dict[str, Dict] = {
    "1h": {
        ...
        "duration_variants": ["1h", "60m", "hourly"],   # primero el más probable
        "label":            "1 hora",
    },
    "4h": {
        "minutes":         240,
...
class MarketScanner:
    ...
    @staticmethod
    def _detect_tf(m: Dict) -> Optional[str]:
        """Detecta timeframe por duración real (start → end)."""
        start = MarketScanner._parse_dt(m, "startDate", "start_date_iso", "startDateIso")
        end   = MarketScanner._parse_dt(m, "endDate",   "end_date_iso",   "endDateIso")
        if not (start and end):
            return None
        dur_min = (end - start).total_seconds() / 60
        if abs(dur_min - 60)  <= 5:  return "1h"
        if abs(dur_min - 240) <= 10: return "4h"
        return None
    ...
    def scan(self, asset: str, tf_key: str, spot: float) -> List[Dict]:
        ...
        # ── MÉTODO 2: Broadcast fallback ──────────────────────────────────────
        if not found:
            log.debug(f"  │  Slug sin resultado → broadcast fallback")
            for m in self._fetch_broadcast(max_hours_ahead=5):
                ...
                end_dt = self._parse_dt(m, "endDate", "end_date_iso")
                if not end_dt:
                    continue
                mins_left = (end_dt - dt_now()).total_seconds() / 60
                if mins_left < 1:
                ...
                tf_det = self._detect_tf(m)
                if tf_det is not None and tf_det != tf_key:
                    continue
                if tf_det is None:
                    if tf_key == "1h" and not (1 <= mins_left <= 65):    continue
                    if tf_key == "4h" and not (1 <= mins_left <= 250):   continue
        ...
        if not found:
            ...
            log.info(f"  └─ {len(found)} mercado(s) válido(s)")
        return found
...
class CircuitBreaker:
    ...
    def check(self, e: PaperEngine) -> bool:
        if e.drawdown_today >= CB_PCT:
            self.tripped = True
            self.reason  = (
                f"Drawdown diario {e.drawdown_today*100:.1f}% ≥ "
                f"límite {CB_PCT*100:.0f}%"
            )
            return True
        return False
...
class TelegramBot:
    ...
    def _handle(self, cmd: str, cid: str, state: BotState):
        ...
        if cmd == "/help":
            ...
            pa_s = "⏸ PAUSADO"  if state.paused       else "▶ ACTIVO"
            ...
            self.send("▶ Bot reanudado.\nCircuit breaker reseteado.", to=cid)
...
def main():
    banner = "\n".join([
        ...
        f"  CB diario:   {CB_PCT*100:.0f}% drawdown",
    ...
    # Loop principal con error handling
    ...
    while True:
        try:
            ...
            if state.cb.tripped:
                log.warning(f"  [CB] {state.cb.reason}")
                time.sleep(CYCLE_SLEEP)
                continue
            ...
            tg.stop()
            tg.send("🛑 Sentinel v8.1 detenido manualmente.")
            discord("🛑 Sentinel v8.1 detenido manualmente.")
            break
